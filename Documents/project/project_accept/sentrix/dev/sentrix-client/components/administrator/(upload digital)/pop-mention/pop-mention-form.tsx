...
import { PopMentionFormProps } from '@/types/components';

import { filterSupportedFiles, getSupportedFileTypesMessage } from '@/utils/upload-file-validator';

const PopMentionForm = ({ onSubmit }: PopMentionFormProps) => {
  ...
  const { mutate, isPending } = useMutation({
    mutationKey: ['create-pop-mention'],
    mutationFn: onSubmit,
    onSuccess: ({ success, message }) => {
      ...
      router.push(`/dashboard/administrator/upload-digital/pop-mention`);
      showMessage(message);
    },
    onError: (error: Error) => {
