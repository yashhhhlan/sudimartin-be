...
import { PopAccountDLFormProps } from '@/types/components';

import { filterSupportedFiles, getSupportedFileTypesMessage } from '@/utils/upload-file-validator';

const PopAccountDLForm = ({ onSubmit }: PopAccountDLFormProps) => {
  ...
  const { mutate, isPending } = useMutation({
    mutationKey: ['create-pop-account-dl'],
    mutationFn: onSubmit,
    onSuccess: ({ success, message }) => {
      ...
      router.push(`/dashboard/administrator/upload-digital/pop-account-dl`);
      showMessage(message);
    },
    onError: (error: Error) => {
...
export default PopAccountDLForm;
