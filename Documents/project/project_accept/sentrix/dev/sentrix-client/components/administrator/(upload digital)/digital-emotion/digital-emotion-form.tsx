...
import { DigitalEmotionFormProps } from '@/types/components';

import { filterSupportedFiles, getSupportedFileTypesMessage } from '@/utils/upload-file-validator';

const DigitalEmotionForm = ({ onSubmit }: DigitalEmotionFormProps) => {
  ...
  const { mutate, isPending } = useMutation({
    mutationKey: ['create-digital-emotion'],
    mutationFn: onSubmit,
    onSuccess: ({ success, message }) => {
      ...
      router.push(`/dashboard/administrator/upload-digital/emotion`);
      showMessage(message);
    },
    onError: (error: Error) => {
...
export default DigitalEmotionForm;
